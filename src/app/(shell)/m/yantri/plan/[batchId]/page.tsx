...
import StatusBadge from "@/components/yantri/StatusBadge";
...
export default function PlanPage() {
  ...
  const loadBatch = useCallback(async () => {
    const res = await fetch(`/api/yantri/trends/batch/${batchId}`);
  ...
  const submitResearch = async (narrativeId: string) => {
    await fetch(`/api/yantri/editorial-narratives/${narrativeId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  if (!batch) return <div className="flex items-center justify-center py-20"><Loader2 className="w-8 h-8 animate-spin text-muted-foreground" /></div>;
        ...
        <div className="flex gap-2">
          {narratives.length === 0 && (
            <Button onClick={runScan} disabled={scanning} className="px-6 py-2.5 rounded-lg">
              {scanning ? <><Loader2 className="w-4 h-4 animate-spin" /> Running Yantri...</> : "Run Yantri"}
            </Button>
          )}
          {narratives.length > 0 && (
            <Button
              ...
              onClick={async () => {
                ...
                await fetch(`/api/yantri/trends/batch/${batchId}`, { method: "PUT" });
                setNarratives([]);
            ...
            {isNewDeliverable && active.status === "producing" && !generating && (
              <div className="space-y-6 mb-4">
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  >
                    {loading.approve ? <><Loader2 className="w-4 h-4 animate-spin" /> Approving...</> : "Approve & Finalize"}
                  </Button>
                  <Button
                    variant="outline"
