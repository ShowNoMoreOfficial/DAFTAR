...
export default function PromptEditorPage() {
  ...
  const fetchTemplate = useCallback(async () => {
    try {
      const res = await fetch(`/api/yantri/prompt-templates/${id}`);
      ...
      router.push("/m/yantri/prompt-library");
    } finally {
      setLoading(false);
    }
  }, [id, router]);
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const url = isNew ? "/api/yantri/prompt-templates" : `/api/yantri/prompt-templates/${id}`;
      ...
      if (isNew) router.push(`/m/yantri/prompt-library/${saved.id}`);
    } catch {
  ...
  async function handleTest() {
    ...
    setTestResult(null);
    try {
      const res = await fetch("/api/yantri/prompt-templates/test", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
