    ...
    <form onSubmit={handleSubmit} className="max-w-2xl space-y-8 animate-fade-in">
      ...
      <div className="flex gap-4 pt-4">
        <Button type="submit" disabled={saving} className="flex-1 py-6 font-black uppercase tracking-widest">
          {saving ? <><Loader2 className="w-4 h-4 animate-spin" /> Syncing...</> : initial?.id ? "Update Architecture" : "Deploy Identity"}
        </Button>
        <Button type="button" variant="outline" onClick={() => router.push("/m/yantri/brands")} className="py-6 px-8 font-black uppercase tracking-widest">
          Cancel
        </Button>
      </div>
    </form>
