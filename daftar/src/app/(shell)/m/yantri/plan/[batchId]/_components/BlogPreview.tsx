...
import CopyButton from "@/components/yantri/CopyButton";
...
export default function BlogPreview({ data }: { data: BlogDeliverable }) {
  ...
  const articleSections = useMemo(() => {
    ...
    const remaining = content.article;
          ...
          <div className="space-y-4">
            <Card className="rounded-xl border-border p-5">
              ...
              {featuredImage ? (
                <>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={featuredImage} alt={postingPlan.banner_description || "Featured image"} className="w-full rounded-lg aspect-video object-cover" />
                ...
                {content.inline_image_prompts.map((ip) => (
                  <Card key={ip.index} className="rounded-xl border-border p-5">
                    ...
                    {inlineImages[ip.index] ? (
                      <>
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={inlineImages[ip.index]} alt={ip.alt} className="w-full rounded-lg aspect-video object-cover" />
                        <p className="text-[10px] text-zinc-600 italic mt-2">{ip.alt}</p>
                      </>
                    ) : (
                      <button onClick={() => handleGenerateInline(ip.index, ip.prompt)} disabled={inlineLoading[ip.index]} className="w-full py-4 bg-zinc-800/50 rounded-lg text-xs text-zinc-400 hover:bg-zinc-800 transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {postingPlan.secondary_keyphrases.map((kp, i) => (
                      <Badge key={i} variant="secondary" className="bg-zinc-800 text-zinc-400 border-none text-xs">{kp}</Badge>
                    ))}
                  </div>
                </div>
              )}
              ...
              <div className="space-y-4">
                ...
                {postingPlan.additional_category && (
                  <div>
                    ...
                    <Badge variant="secondary" className="bg-zinc-800 text-zinc-400 border-none">{postingPlan.additional_category}</Badge>
                  </div>
                )}
                <div>
              ...
              <Card className="rounded-xl border-border p-5">
                ...
                <div className="border border-zinc-700 rounded-lg overflow-hidden max-w-md">
                  ...
                  <div className="p-3 bg-zinc-800">
                    <p className="text-sm font-semibold text-zinc-200 line-clamp-2">{postingPlan.og_title || postingPlan.seo_title}</p>
                    <p className="text-xs text-zinc-500 mt-1 line-clamp-2">{postingPlan.og_description || postingPlan.meta_description}</p>
                  </div>
                </div>
              </Card>
            )}
