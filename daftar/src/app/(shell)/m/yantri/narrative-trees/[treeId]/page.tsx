...
export default function NarrativeTreeDetailPage() {
  ...
  const fetchTree = useCallback(async () => {
    try {
      const res = await fetch(`/api/yantri/narrative-trees/${treeId}`);
      if (!res.ok) {
        if (res.status === 404) {
  ...
  useEffect(() => {
    ...
    fetch("/api/yantri/narrative-trees")
      .then((r) => r.json())
      .then((data) => {
  ...
  async function addHypothesis() {
    ...
    try {
      const res = await fetch(`/api/yantri/narrative-trees/${treeId}/hypothesis`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleMerge(sourceId: string, targetId: string) {
    ...
    try {
      const res = await fetch("/api/yantri/narrative-trees/merge", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      router.push(`/m/yantri/narrative-trees/${targetId}`);
    } catch {
