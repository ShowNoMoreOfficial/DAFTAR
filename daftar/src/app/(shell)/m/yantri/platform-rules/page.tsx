...
export default function PlatformRulesPage() {
  ...
  useEffect(() => {
    fetch("/api/yantri/platform-rules")
      .then((r) => r.json())
      .then(setRules);
  }, []);
  ...
  const saveEdit = async (id: string) => {
    const res = await fetch(`/api/yantri/platform-rules/${id}`, {
      method: "PUT",
  ...
  const deleteRule = async (id: string) => {
    await fetch(`/api/yantri/platform-rules/${id}`, { method: "DELETE" });
    setRules(rules.filter((r) => r.id !== id));
  };

  const addRule = async () => {
    const res = await fetch("/api/yantri/platform-rules", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
